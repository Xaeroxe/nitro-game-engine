...
use engine_filters::SignalWhenOver;
...
pub struct Handle {
    ...
    // Updates to true when the last sound is over and this handle is idle.
...
impl Handle {
              ...
              S::Item: Sample + Clone + Send
    {
        ...
        *self.end.lock().unwrap() = Some(rx);
        ...
        let source = SignalWhenOver::new(source, is_over.clone());
        self.is_empty = is_over;
